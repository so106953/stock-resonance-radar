        ...
        <aside className="watchlist">
          ...
          <div className="refresh-board" aria-live="polite">
            {[["signals", "盘中候选"], ["confirmed", "收盘确认"], ["watchlist", "我的自选"]].map(([key, label]) => {
              ...
              return <button type="button" key={key} className={`refresh-item refresh-control ${status.state} ${listMode === key ? "selected" : ""}`} aria-label={`单独刷新${label}`} disabled={scanning || fullScan.status === "running"} onClick={() => { setListMode(key); refreshSection(key).catch(() => {}); }}><span className="refresh-state-icon">{status.state === "running" ? <ArrowClockwise className="spin" size={14}/> : status.state === "error" ? <Warning size={14}/> : status.updatedAt ? <Check size={14}/> : <ArrowClockwise size={14}/>}</span><div><strong>{label}{status.state === "running" && determinate ? ` ${status.progress}%` : ""}</strong><small>{status.state === "running" ? status.message || "刷新中" : status.updatedAt ? refreshLabel(key) : "点击单独刷新"}</small><span className={`refresh-progress ${status.state === "running" && !determinate ? "indeterminate" : ""}`} aria-label={status.state === "running" ? `${label}刷新进度` : undefined} aria-valuenow={determinate ? status.progress : undefined} role={status.state === "running" ? "progressbar" : undefined}><i style={{ width: `${determinate ? status.progress : status.state === "done" ? 100 : 0}%` }} /></span></div></button>;
            })}
          </div>
